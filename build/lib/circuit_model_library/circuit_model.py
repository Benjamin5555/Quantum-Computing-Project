...
class QuantumCircuit():
    ...
    def __init__(self, circuit_string_list, gates_dictionary):
        ...
        for x in circuit_string_list:
            returnList.append(list(x[::-1])) # Reverses the string in place and convert to actual list 
                                             # e.g. "ABC" -> ['A','B','C']
        print(returnList) 
        ...
        circuit_matrix_comp = []
        print(circuit_string_list)
        for gate_column in circuit_string_list:# Go through whole list for each column i.e. a line of gates 

            current_column = []

            for gate_id in gate_column: #Go through the whole column and replace each gate_id with 
                                        # actual gate matrix
                current_column.append(gates_dictionary[gate_id]) #'Replace the gate_id with the 
                                                                 # actual gate
            
            #Get tensor product of each 'columns' (At this point not actually columns just a list of
            #gates) but were originally columns in the circuit
